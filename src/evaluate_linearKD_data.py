# ファイル名: evaluate_linearKD_data.py
# 内容: KD対応データセット定義、およびHuggingFaceからの実データロード処理
...
from evaluate_linearKD_config import IMG_SIZE
...
class KDImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        soft_target = torch.empty(0)
        if isinstance(item, str):
            pt_path = item.rsplit('.', 1)[0] + '.pt'
            if os.path.exists(pt_path):
                try:
                    soft_target = torch.load(pt_path, map_location='cpu')
                except Exception:
                    pass

        return img, lbl, soft_target
...
# ========================================================
# HuggingFace Real Data Loading Logic
# ========================================================
