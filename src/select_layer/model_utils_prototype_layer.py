# =========================
# src/model_utils_prototype_layer.py
...
class EncoderClassifier(nn.Module):
    def __init__(
        ...
        if self.use_timm_encoder:
            ...
            is_clip = self.encoder_model_name.startswith("openai/clip") or "laion" in self.encoder_model_name
            is_siglip = "siglip" in self.encoder_model_name
            is_dinov2 = "dinov2" in self.encoder_model_name
            is_dino = (
                "dino" in self.encoder_model_name
                and "v3" not in self.encoder_model_name
                and "v2" not in self.encoder_model_name
            ...
            # --------------------------------------------
            # CLIP系は最終層だけ強制的に pooler を使う
            # 中間層では pooler が使えないため cls / mean に寄せる
            # --------------------------------------------
            if is_clip:
                if self.feature_layer == -1:
                    self.feature_source = "pooler"
                else:
                    self.feature_source = "mean" if feature_source == "mean" else "cls"
...
class MultiModelGM:
    ...
    def _build_real_batch(self, real_image_pool_cpu, target_count):
        ...
        real_batch = selected.repeat_interleave(aug_per_image, dim=0)  # [K*A, 3, H, W]
    ...
    def _build_all_real_batches(self, real_image_pools_cpu, target_count):
        """
        各クラスの real batch を作り, まとめて返す.
        戻り値:
            real_batch_all: [B*target_count, 3, H, W]
        """
        batch_list = []
        for b_idx in range(self.batch_size):
            ...
            batch_list.append(real_batch)

        real_batch_all = torch.cat(batch_list, dim=0)
        return real_batch_all

    def _build_augmented_syn_batch(self, syn_image):
        """
        syn_image: [B, 3, H, W]
        戻り値:
            augmented_batch: [B*syn_aug_num, 3, H, W]
        """
        augmented_list = []
    ...
    def optimize_step(self, real_image_pools_cpu):
        ...
        # --------------------------------------------------
        # 1. syn 側 augmentation をまとめて作成
        # --------------------------------------------------
        augmented_batch = self._build_augmented_syn_batch(syn_image)  # [B*S, 3, H, W]
        inp_syn = self.preprocess(augmented_batch)

        # --------------------------------------------------
        # 2. real 側 batch も class ごとに作って, まとめる
        # --------------------------------------------------
        real_batch_all = self._build_all_real_batches(
            real_image_pools_cpu=real_image_pools_cpu,
            target_count=self.syn_aug_num,
        )  # [B*S, 3, H, W]
        inp_real = self.preprocess(real_batch_all)
        ...
        for i, model in enumerate(self.models):
            ...
            try:
                with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                    syn_feats_all = model.forward_features(inp_syn)   # [B*S, D]
                    real_feats_all = model.forward_features(inp_real) # [B*S, D]
            except Exception:
                with autocast():
                    syn_feats_all = model.forward_features(inp_syn)
                    real_feats_all = model.forward_features(inp_real)
            ...
            for b_idx, tgt_cls in enumerate(self.target_classes):
                ...
                real_feats = real_feats_all[start_idx:end_idx]
                ...
                real_proto = F.normalize(real_feats.detach().mean(dim=0, keepdim=True), dim=1)
                syn_proto = F.normalize(syn_feats.mean(dim=0, keepdim=True), dim=1)
                proto_loss_k = 1.0 - F.cosine_similarity(real_proto, syn_proto).mean()
                total_proto_loss += (proto_loss_k * weight) / self.batch_size
        ...
        weight_proto = float(getattr(self.args, "weight_proto", 0.0))
        total_loss = (
            (total_grad_loss * self.args.weight_grad)
            + (total_proto_loss * weight_proto)
            + (loss_tv * self.args.weight_tv)
        )
        ...
        return total_grad_loss.item(), total_proto_loss.item(), loss_tv.item(), per_model_sims, total_loss.item()
    ...
    def run(self, real_image_pools_cpu, save_dirs, class_names, logger, global_pbar, gen_idx=0):
        ...
        for i in local_pbar:
            ...
            l_grad, l_proto, l_tv, model_sims, current_total_loss = self.optimize_step(real_image_pools_cpu)
