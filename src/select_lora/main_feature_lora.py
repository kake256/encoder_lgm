...
def configure_second_order_grad_attention(logger=None):
    """
    LoRA勾配一致では2階微分(create_graph=True)を使うため、
    Flash/Memory-efficient SDPを無効化してmath kernelへフォールバックする。
    """
    if not torch.cuda.is_available():
        if logger:
            logger.log("CUDA is not available; SDP kernel configuration skipped.")
        return
    ...
    _set_flag("enable_flash_sdp", False)
    _set_flag("enable_mem_efficient_sdp", False)
    _set_flag("enable_math_sdp", True)
    _set_flag("enable_cudnn_sdp", False)

    if logger:
        if changed:
            logger.log("Configured SDP for 2nd-order gradients: " + ", ".join(changed))
        else:
            logger.log("SDP backend flags were not found on this torch build.")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="lora",
        choices=["classifier", "lora", "hybrid"],
        help="Gradient matching target parameters"
    )
    parser.add_argument("--hybrid_lambda", type=float, default=0.5, help="hybrid mode weight for LoRA grads")
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["q_proj", "v_proj", "query", "value", "qkv"],
        help="Linear submodule names where LoRA will be attached"
    )
    parser.add_argument(
        "--lora_last_n_blocks",
        type=int,
        default=2,
        help="Attach LoRA only to the last N transformer blocks (if block index is available)"
    )
    parser.add_argument(
        "--lora_reset_each_step",
        action="store_true",
        help="Reinitialize LoRA weights at every distillation step"
    )
...
if __name__ == "__main__":
    ...
    configure_second_order_grad_attention(logger)
    ...
    logger.log(
        ...
        f"grad_target={args.grad_target}, "
        f"lora_rank={args.lora_rank}, "
        f"lora_alpha={args.lora_alpha}, "
        f"lora_last_n_blocks={args.lora_last_n_blocks}, "
        f"lora_reset_each_step={args.lora_reset_each_step}"
    )
