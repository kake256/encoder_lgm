...
# 内容: 全学習モード対応・KD切り替え・実画像ベースライン・自動ロジット抽出・キャッシュ＆ES・リアルタイムCSV保存・実行順序制御対応版
...
# ========================================================
# 3. Data Loading (Fast Memory Load)
# ========================================================
...
def load_syn_data_to_memory(image_paths, labels, load_logits=False, img_size=224):
    ...
    for img_path in tqdm(image_paths, desc="Loading Syn Data", leave=False):
        try:
            img = Image.open(img_path).convert("RGB")
...
def convert_pil_to_tensor_memory(images, labels, img_size=224):
    transform = T.Compose([T.Resize((img_size, img_size)), T.ToTensor()])
    tensor_list = []
    for img in tqdm(images, desc="Processing Real Data", leave=False):
        try:
            tensor_img = transform(img)
        except Exception:
            tensor_img = torch.zeros(3, img_size, img_size)
...
def train_fast_unified(model, train_tensors, test_loader, mode, config, use_kd=False, cached_test_data=None):
    images, labels, soft_targets = train_tensors
    ...
    batch_size = min(len(images), int(config.get("batch_size", 128)))
        ...
        if (epoch + 1) % val_interval == 0 or (epoch + 1) == epochs:
            ...
            with torch.no_grad(), autocast():
                if cached_test_data is not None:
                    ...
                    acc = accuracy_score(test_lbls.cpu().numpy(), test_logits.argmax(1).cpu().numpy())
                else:
                    preds, targets = [], []
                    for tx, ty in test_loader:
                        preds.extend(model(tx.to(DEVICE)).argmax(1).cpu().numpy())
                        targets.extend(ty.numpy())
    ...
    with torch.no_grad(), autocast():
        ...
        for i in range(num_batches):
            ...
            x_b = (images[s:e] - mean) / std
            tr_preds.extend(model(x_b).argmax(1).cpu().numpy())
            tr_targets.extend(labels[s:e].cpu().numpy())
...
def get_sort_weight(exp_name):
    """
    指定された文字列(exp_name)に対するソート順のウェイトを返します。
    数値が小さいほど先に実行されます。
    """
    name = exp_name.lower()
...
def main():
    ...
    def save_result_realtime(result_dict):
        ...
        # ファイルが存在しなければヘッダー付きで作成、存在すれば追記(ヘッダーなし)
    ...
    for eval_name in args.evaluators:
        ...
        def run_exp(name, imgs, lbls, is_real=False):
            ...
            if is_real:
                images_tensor, labels_tensor, logits_tensor = convert_pil_to_tensor_memory(imgs, lbls)
                current_use_kd = False
            else:
                images_tensor, labels_tensor, logits_tensor = load_syn_data_to_memory(imgs, lbls, load_logits=use_kd)
                current_use_kd = use_kd
            
            for t in range(args.num_trials):
                set_seed(SEED + t)
                trainable_model = TrainableModel(extractor, num_classes).to(DEVICE)
                acc, t_acc = train_fast_unified(trainable_model, (images_tensor, labels_tensor, logits_tensor), test_loader, args.mode, train_config, use_kd=current_use_kd, cached_test_data=cached_test_data)
        ...
        # 2. Synthetic Data (Individual) をキューに追加
        if experiments:
            for exp_name, data in experiments.items():
