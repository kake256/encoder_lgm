...
def train_fast_unified(model, train_data, test_loader, mode, config, use_kd=False, cached_test_data=None, is_precomputed=False):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % val_interval == 0 or (epoch + 1) == epochs:
            ...
            if acc >= best_acc:
                best_acc = acc
                patience_counter = 0
                best_preds_cache = all_preds
                best_lbls_cache = all_lbls
            else:
                patience_counter += 1
    ...
    class_wise_acc = {}
    if 'best_preds_cache' in locals():
        for c in torch.unique(best_lbls_cache):
            c_idx = c.item()
            mask = (best_lbls_cache == c)
            c_acc = (best_preds_cache[mask] == best_lbls_cache[mask]).float().mean().item()
            class_wise_acc[c_idx] = c_acc
...
def main():
    ...
    for eval_name in args.evaluators:
        ...
        def run_exp(name, imgs, lbls, is_real=False):
            ...
            for t in range(args.num_trials):
                ...
                for c_id, c_acc in c_acc_dict.items():
                    sum_class_accs[c_id] = sum_class_accs.get(c_id, 0.0) + c_acc
                    class_counts[c_id] = class_counts.get(c_id, 0) + 1

            trial_accs_pct = [a * 100.0 for a in trial_accs]
            trial_t_accs_pct = [t_a * 100.0 for t_a in trial_t_accs]
            mean_acc = float(np.mean(trial_accs_pct))
            std_acc = float(np.std(trial_accs_pct))
    ...
    print(f"Class-wise Accuracies saved to: {class_csv_path}")
