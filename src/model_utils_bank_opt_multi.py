...
# ==========================================================
# 3. 生成器 (PyramidGenerator) - ★バッチサイズ対応に変更
# ==========================================================
class PyramidGenerator(nn.Module):
    def __init__(self, target_size=224, start_size=16, activation="sigmoid", initial_image=None, noise_level=0.0, batch_size=1):
        super().__init__()
        ...
        if initial_image is not None:
            ...
            # initial_imageをバッチサイズ分複製する
            init_val = init_val.repeat(self.batch_size, 1, 1, 1)
            self.levels = nn.ParameterList([nn.Parameter(init_val)])
        else:
            self.levels = nn.ParameterList([nn.Parameter(torch.randn(self.batch_size, 3, start_size, start_size) * 0.1)])
...
class FeatureBankSystem:
    ...
    def create_or_load_bank(self, models, raw_images_pool_cpu: torch.Tensor, target_class: int, logger):
        ...
        target_total_vectors = int(self.args.real_aug)
        ...
        for i, model in enumerate(models):
            ...
            # 1. まずキャッシュが存在するか確認し、あれば読み込む（画像枚数0でもOK！）
            if os.path.exists(cache_path):
                try:
                    feats_cpu = torch.load(cache_path, map_location="cpu")
                    logger.log(f"  [Cache Hit] {model_name}")
                    bank_list.append(feats_cpu)
                    continue
                except Exception as e:
                    logger.log(f"  [Cache Error] Failed to load {cache_path}: {e}. Recomputing...")

            # 2. キャッシュが存在せず、計算が必要なのに画像がない場合はエラーを回避して空を返す
            if num_images == 0:
                logger.log(f"  [Warning] No images found for class {target_class}! Cannot compute cache.")
                bank_list.append(torch.empty(0))
                continue

            # 3. キャッシュ計算処理 (動的拡張)
            dynamic_aug_multiplier = max(1, round(target_total_vectors / num_images))
            total_vectors = num_images * dynamic_aug_multiplier
            logger.log(f"  Class {target_class}: {num_images} real images. "
                       f"Multiplier dynamically set to {dynamic_aug_multiplier}x -> Total vectors: {total_vectors}")
            ...
            with torch.no_grad():
                with tqdm(total=total_vectors, desc=f"  Comp {model_name}", leave=True, unit="img") as pbar:
                    for _ in range(dynamic_aug_multiplier):
            ...
            logger.log(f"  Saved cache.")
            bank_list.append(final_bank_tensor)

        return bank_list

# ==========================================================
# 5. メイン最適化クラス - ★バッチ処理対応に変更
# ==========================================================
class MultiModelGM:
    def __init__(self, models, model_weights, target_classes, args, device, initial_image=None):
        ...
        # ★ target_class をリストで受け取りバッチサイズを決定
        ...
        self.generator = PyramidGenerator(
            ...
            batch_size=self.batch_size, # ★ 並列生成
        ).to(device)
    ...
    def get_grads_from_features(self, model, features, tgt_class, create_graph=False):
        ...
        # ★ 各バッチ要素用の正解ラベルを動的に生成
        target_labels = torch.tensor([tgt_class] * features.size(0), device=self.device)
    ...
    def optimize_step(self, feature_bank_list_gpu):
        self.optimizer.zero_grad()
        syn_image = self.generator() # 形状: [B, 3, 224, 224] (Bはクラス数=10)
        
        # ▼▼▼ 劇的スピードアップ ＋ 完全な多様性の両立 ▼▼▼
        # 320回ループするのではなく、バッチ(10クラス)をまとめて32回だけループする
        augmented_list = []
        for _ in range(self.syn_aug_num):
            # 10クラス同時に1つのランダム変化を適用（これを32回繰り返す）
            augmented_list.append(self.syn_augmentor(syn_image))
            
        # リストを結合して [B, 32, 3, 224, 224] にし、[B*32, 3, 224, 224] に平坦化する
        # （これにより、元のコードのクラスごとの切り分け処理と完璧に一致します）
        augmented_batch = torch.stack(augmented_list, dim=1).reshape(-1, 3, self.args.image_size, self.args.image_size)
        
        inp_syn = self.preprocess(augmented_batch)
        # ▲▲▲ 修正ここまで ▲▲▲

        total_grad_loss = 0.0
        per_model_sims = {self.args.encoder_names[i]: 0.0 for i in range(len(self.models)) if self.model_weights[i] > 0}
        ...
        for i, model in enumerate(self.models):
            ...
            for b_idx, tgt_cls in enumerate(self.target_classes):
                ...
                # 該当クラスの特徴量だけをスライスして取り出す
                start_idx = b_idx * self.syn_aug_num
                end_idx = start_idx + self.syn_aug_num
                syn_feats = syn_feats_all[start_idx:end_idx]

                # Feature bankからサンプリング
                bank_gpu = feature_bank_list_gpu[b_idx][i]
                idx_gpu = torch.randint(0, bank_gpu.size(0), (self.syn_aug_num,), device=self.device)
                real_feats = bank_gpu.index_select(0, idx_gpu)

                try:
                    with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                        target_grad = self.get_grads_from_features(model, real_feats, tgt_cls, create_graph=False)
                        syn_grad = self.get_grads_from_features(model, syn_feats, tgt_cls, create_graph=True)
                except Exception:
                    with autocast():
                        target_grad = self.get_grads_from_features(model, real_feats, tgt_cls, create_graph=False)
                        syn_grad = self.get_grads_from_features(model, syn_feats, tgt_cls, create_graph=True)
    ...
    def run(self, feature_bank_list_cpu, save_dirs, class_names, logger, global_pbar, gen_idx=0):
        # feature_bank_list_cpu は [B][num_models] のリスト。これをGPUへ転送
        feature_bank_list_gpu = []
        for b_idx in range(self.batch_size):
            banks_for_class = [fb.to(self.device, non_blocking=True) for fb in feature_bank_list_cpu[b_idx]]
            feature_bank_list_gpu.append(banks_for_class)
        
        logger.log(f"[Batch Size {self.batch_size}][Gen {gen_idx}] Optimization Start.")
        loss_history = []
        best_loss = float("inf")
        ...
        for i in local_pbar:
            ...
            # Pass GPU banks directly
            l_grad, l_tv, model_sims, current_total_loss = self.optimize_step(feature_bank_list_gpu)
            ...
            global_pbar.update(self.batch_size) # 進捗バーをバッチサイズ分進める

            if i % 500 == 0:
                with torch.no_grad():
                    current_imgs = self.generator().detach().cpu()
                    # ★ バッチ内の各画像をそれぞれ対応するディレクトリに保存
