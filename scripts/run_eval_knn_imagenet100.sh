...
SELECTED_EVALUATORS="ResNet50 MAE OpenCLIP_RN50 DINOv1 DINOv2 CLIP SigLIP" # Debug用
...
BASELINE_SHOTS="1,5" #,200,1500"
# ロードする実画像の最大数 (BASELINE_SHOTSの最大値以上にする必要があります)
MAX_TRAIN_SAMPLES=50 #1500
...
# Example: CUB
DEFAULT_DATASET_DIR="$PROJECT_ROOT/makeData/dataset_clean_imagenet100_FULL_batch"
DEFAULT_DATASET_TYPE="imagenet100"
...
DEFAULT_IMAGENET100_HF="clane9/imagenet-100"
DEFAULT_IMAGENET100_SPLIT="validation"
...
IMAGENET100_HF="$DEFAULT_IMAGENET100_HF"
IMAGENET100_SPLIT="$DEFAULT_IMAGENET100_SPLIT"
...
if [ "$DATASET_TYPE" = "cub" ]; then
  ...
  python3 "$PYTHON_SCRIPT" "$DATASET_DIR" \
    --output_dir "$OUTPUT_DIR" \
    --dataset_type "$DATASET_TYPE" \
    --hf_dataset "$IMAGENET100_HF" \
    --hf_split "$IMAGENET100_SPLIT" \
    --k_neighbors "$K_NEIGHBORS" \
    --baseline_shots "$BASELINE_SHOTS" \
    --max_train_samples "$MAX_TRAIN_SAMPLES" \
    --save_analysis \
