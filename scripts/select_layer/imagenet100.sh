#!/bin/bash
# run_1gpu_imagenet100_multi_layer.sh
# 1x GPU向け: ImageNet-100 を同一GPU上の複数ワーカーで並列最適化
set -e
...
# 出力先
EXPERIMENT_ROOT_DIR="$RESULTS_BASE_DIR/gene_experiment_imagenet100_FULL"
mkdir -p "$EXPERIMENT_ROOT_DIR"

# 生成後に学習用データセット形式へ整理する設定
ORGANIZE_DATASET_AFTER_GEN=1
ORGANIZED_DATASET_DIR="$PROJECT_DIR/makeData/dataset_clean_imagenet100_FULL"
CLEAN_ORGANIZED_DATASET=1
...
PROJ_DIM_LIST=("1024 1024 1024 1024")
...
run_generation_worker() {
    ...
    local BASE_CMD="CUDA_VISIBLE_DEVICES=$GPU_ID_FOR_ALL_WORKERS python3 \"$SRC_DIR/select_layer/main_feature_layer.py\" \
...
echo " feature_layer=$FEATURE_LAYER, feature_source=$FEATURE_SOURCE"
...
if [ "$ORGANIZE_DATASET_AFTER_GEN" -eq 1 ]; then
  echo ">>> Organizing generated images for training"
  echo " Source: $BASE_OUTPUT_DIR"
  echo " Target: $ORGANIZED_DATASET_DIR"

  ORGANIZE_CLEAN_FLAG=""
  if [ "$CLEAN_ORGANIZED_DATASET" -eq 1 ]; then
    ORGANIZE_CLEAN_FLAG="--clean"
  fi

  python3 "$SRC_DIR/organize_dataset_manual.py" \
    --input "$BASE_OUTPUT_DIR" \
    --output "$ORGANIZED_DATASET_DIR" \
    $ORGANIZE_CLEAN_FLAG
fi

echo "All phases completed. Results at: $BASE_OUTPUT_DIR"
if [ "$ORGANIZE_DATASET_AFTER_GEN" -eq 1 ]; then
  echo "Organized dataset at: $ORGANIZED_DATASET_DIR"
fi
